...
@pytest_asyncio.fixture(scope="module")
async def unfair_user() -> User:
    """Return user instance for access control tests."""
    return await AsyncUserFactory.create()


@pytest_asyncio.fixture(scope="module")
async def builded_test_user() -> User:
    """Return builded user instance for tests."""
    return await AsyncUserFactory.build()
...
@pytest_asyncio.fixture(scope="module")
async def unfair_user_token(unfair_user: User) -> str:
    """Return test unfair user auth token."""
    return create_access_token(data={"sub": unfair_user.username})


@pytest_asyncio.fixture(scope="module")
async def unfair_authorized_async_client(
    async_client: AsyncClient,
    unfair_user_token: str,
) -> AsyncClient:
    """Return and provide token to async client for access control tests."""
    async_client.headers.update(
        {
            "Authorization": f"Bearer {unfair_user_token}",
        },
    )
    return async_client
